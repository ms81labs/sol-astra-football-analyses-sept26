...
DEFAULT_INPUT_BATCH_NAME = "v7_1_crop_manifest_consistency_refresh_v1"
DEFAULT_OUTPUT_DIR_NAME = "v7_1_export_label_overlay_audit_v1"
DEFAULT_VIDEO_PATH = REPO_ROOT / "videos" / "trimed-5min.mp4"

BLOCKER_POSITIVE_LABEL_MISSING = "v7_1_export_positive_label_missing"
BLOCKER_NEGATIVE_LABEL_NOT_EMPTY = "v7_1_export_negative_label_not_empty"
BLOCKER_CANARY_LEAKAGE = "v7_1_export_canary_leakage"
BLOCKER_SPLIT_LEAKAGE = "v7_1_export_split_leakage_regression"
BLOCKER_YOLO_INVALID = "v7_1_export_yolo_normalization_invalid"
BLOCKER_ROUNDTRIP = "v7_1_export_bbox_roundtrip_error"
BLOCKER_CLASS_ID = "v7_1_export_class_id_mismatch"
BLOCKER_CROP_SIZE = "v7_1_export_crop_size_mismatch"
BLOCKER_OVERLAY_MISSING = "v7_1_export_overlay_review_missing"
BLOCKER_UNSAFE_FULL_FRAME = "v7_1_export_unsafe_full_frame_negative_leak"

READY_CLASS = "v7_1_export_overlay_audit_ready"
NEXT_TINY_OVERFIT = "v7_1_tiny_overfit_sanity_train"
NEXT_POSITIVE_FIX = "v7_1_positive_crop_transform_fix"
NEXT_MANIFEST_REFRESH = "v7_1_crop_manifest_consistency_refresh"
NEXT_NEGATIVE_REVIEW = "v7_1_negative_crop_review_or_safety_audit"
NEXT_MANUAL_REVIEW = "v7_1_manual_overlay_review"
...
def _classify(summary: dict[str, Any]) -> tuple[str | None, str | None, bool, str]:
    ...
    return None, NEXT_TINY_OVERFIT, True, "Export and overlay audit passed. Advance to a tiny overfit sanity train before any full v7.1 training run."


def _markdown_summary(summary: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# V7.1 Export Label Overlay Audit",
            "",
...
def _export_positive(
    ...
    *,
    ...
    if frame is None or source_bbox is None or crop_bbox is None or bounds is None:
        ...
        return result
    crop, crop_bounds_int, crop_status = _crop_image(frame, bounds)
    if crop is None:
...
def run_v7_1_export_label_overlay_audit(
    *,
    ...
    manifest = _load_json(input_root / "v7_1_local_crop_training_manifest.json")
    ...
    positive_sheet_count = _make_contact_sheet(
        positive_rows,
        output_path=output_root / "v7_1_overlay_contact_sheet_positive.jpg",
        title="positive",
        max_items=90,
    )
    negative_sheet_count = _make_contact_sheet(
        negative_rows,
        output_path=output_root / "v7_1_overlay_contact_sheet_negative.jpg",
        title="negative",
        max_items=60,
    )
    canary_sheet_count = _make_contact_sheet(
        canary_rows,
        output_path=output_root / "v7_1_overlay_contact_sheet_canary.jpg",
        title="canary",
        max_items=20,
    )
    ...
    summary: dict[str, Any] = {
        "batchName": "v7_1_export_label_overlay_audit",
    ...
    overlay_manifest = {
        ...
        "positiveContactSheetPath": str(output_root / "v7_1_overlay_contact_sheet_positive.jpg"),
        "negativeContactSheetPath": str(output_root / "v7_1_overlay_contact_sheet_negative.jpg"),
        "canaryContactSheetPath": str(output_root / "v7_1_overlay_contact_sheet_canary.jpg"),
    ...
    _write_json(output_root / "v7_1_label_overlay_audit.json", summary)
    _write_json(output_root / "v7_1_crop_label_transform_audit.json", transform_audit)
    _write_json(output_root / "v7_1_export_manifest_consistency_audit.json", consistency_audit)
    _write_json(output_root / "v7_1_overlay_review_manifest.json", overlay_manifest)
...
def main() -> None:
    ...
    payload = run_v7_1_export_label_overlay_audit(
