...
DEFAULT_SOURCE_DIR_NAME = "football_external_soccertrack_analysis_product_ui_binding_v1"
DEFAULT_OUTPUT_DIR_NAME = "football_external_soccertrack_analysis_product_ui_route_implementation_v1"

BLOCKER_UI_BINDING_MISSING = "football_external_soccertrack_analysis_product_ui_binding_missing"
BLOCKER_ROUTE_CONTRACT_GAP = "football_external_soccertrack_analysis_product_ui_route_contract_gap"

NEXT_UI_BINDING = "football_external_soccertrack_analysis_product_ui_binding"
NEXT_ROUTE_CONTRACT_REPAIR = "football_external_soccertrack_analysis_product_ui_route_contract_repair"
...
def _attempt_plan() -> list[dict[str, Any]]:
    return [
        {
            "attemptNumber": 1,
            "attemptApproachFamily": "soccertrack_analysis_product_ui_route_smoke",
            "successCriteria": [
                "serve the saved UI view model from /api/external/soccertrack/{match_id}/analysis",
                "serve the saved HTML smoke from /external/soccertrack/{match_id}/analysis",
        ...
        {
            "attemptNumber": 2,
            "attemptApproachFamily": "soccertrack_analysis_product_ui_route_contract_repair",
            "successCriteria": [
        ...
        {
            "attemptNumber": 3,
            "attemptApproachFamily": "soccertrack_analysis_product_ui_route_blocker_summary",
            "successCriteria": [
                "write blocker truth",
...
def _ui_binding_ready(inputs: dict[str, Any]) -> bool:
    ...
    return bool(
        ...
        and summary.get("selectedMatchId")
        ...
        and route_contract.get("apiRoutePath") == f"/api/external/soccertrack/{summary.get('selectedMatchId')}/analysis"
        and route_contract.get("routePath") == f"/external/soccertrack/{summary.get('selectedMatchId')}/analysis"
        ...
        and view_model.get("schemaVersion") == "soccertrack_analysis_product_ui_view_model_v1"
        and readiness.get("analysisProductUiReady") is True
...
async def _smoke_routes_async(storage_root: Path, match_id: str | None) -> dict[str, Any]:
    match_id = match_id or "unknown"
    app = create_app(storage_root=storage_root)
    transport = httpx.ASGITransport(app=app)
    async with httpx.AsyncClient(transport=transport, base_url="http://127.0.0.1") as client:
        api_response = await client.get(f"/api/external/soccertrack/{match_id}/analysis")
        html_response = await client.get(f"/external/soccertrack/{match_id}/analysis")
    ...
    return {
        ...
        "htmlRoutePath": f"/external/soccertrack/{match_id}/analysis",
...
def _smoke_routes(storage_root: Path, match_id: str | None) -> dict[str, Any]:
    return anyio.run(_smoke_routes_async, storage_root, match_id)
...
def _classify(binding_ready: bool, audit: dict[str, Any]) -> tuple[str | None, str, bool, bool, str]:
    if not binding_ready:
        return (
            ...
            False,
            False,
            "SoccerTrack product UI binding is missing or unsafe; rerun UI binding before route implementation.",
        )
    if audit.get("apiRouteSmokePassed") is not True or audit.get("htmlRouteSmokePassed") is not True:
        return (
            ...
            False,
            True,
            "SoccerTrack product UI route smoke failed or overclaimed readiness.",
        )
    return (
        None,
        ...
        True,
        True,
        "SoccerTrack analysis product route is live from saved artifacts. Advance to product lane closeout; keep detector evaluation, training, promotion, and runtime mutation separate.",
    )
...
def _markdown_summary(summary: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# Football External SoccerTrack Analysis Product UI Route Implementation",
            "",
...
def run_football_external_soccertrack_analysis_product_ui_route_implementation(
    *,
    ...
    attempt_approach_family: str = "soccertrack_analysis_product_ui_route_smoke",
) -> dict[str, Any]:
    ...
    selected_match_id = str(summary_input.get("selectedMatchId") or "unknown")
    smoke = _smoke_routes(storage_root, selected_match_id)
    ...
    summary: dict[str, Any] = {
        ...
        "sourceBatch": "football_external_soccertrack_analysis_product_ui_binding",
        "productUiRouteReady": goal_achieved,
        "selectedMatchId": selected_match_id if selected_match_id != "unknown" else None,
        "apiRoutePath": smoke.get("apiRoutePath"),
        "htmlRoutePath": smoke.get("htmlRoutePath"),
        "reportedEventCount": summary_input.get("reportedEventCount"),
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Smoke the read-only SoccerTrack external analysis UI/API routes.")
    parser.add_argument("--storage-root", type=Path, default=DEFAULT_STORAGE_ROOT)
    parser.add_argument("--candidate-name", default=DEFAULT_CANDIDATE_NAME)
    parser.add_argument("--source-dir-name", default=DEFAULT_SOURCE_DIR_NAME)
    parser.add_argument("--output-dir-name", default=DEFAULT_OUTPUT_DIR_NAME)
    parser.add_argument("--attempt-number", type=int, default=1)
    parser.add_argument("--attempt-approach-family", default="soccertrack_analysis_product_ui_route_smoke")
    return parser


def main() -> None:
    args = _build_parser().parse_args()
    payload = run_football_external_soccertrack_analysis_product_ui_route_implementation(
