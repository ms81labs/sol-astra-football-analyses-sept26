...
DEFAULT_INPUT_BATCH_NAME = "v7_3_bounded_retrain_v1"
DEFAULT_OUTPUT_DIR_NAME = "v7_3_crop_probe_precision_guardrail_audit_v1"
SELECTED_AUDIT_CONF = 0.10

BLOCKER_CHECKPOINT = "v7_3_crop_probe_checkpoint_contract_failure"
BLOCKER_POSITIVE_REGRESSION = "v7_3_crop_probe_positive_localization_regression"
BLOCKER_VAL_RECALL = "v7_3_crop_probe_validation_recall_insufficient"
BLOCKER_NEGATIVE_FLOOD = "v7_3_crop_probe_hard_negative_false_positive_flood"
BLOCKER_CANARY_FLOOD = "v7_3_crop_probe_canary_false_positive_flood"
BLOCKER_TOP_LEFT = "v7_3_crop_probe_top_left_artifact_regression"
BLOCKER_GIANT = "v7_3_crop_probe_giant_box_regression"
BLOCKER_LOW_CONF = "v7_3_crop_probe_low_confidence_flood_regression"
BLOCKER_ARTIFACTS = "v7_3_crop_probe_overlay_artifacts_missing"

NEXT_PIPELINE = "v7_3_full_pipeline_non_promotion_eval"
...
def _attempt_plan() -> dict[str, Any]:
    return {
        ...
        "attempts": [
            {
                "attemptNumber": 1,
                "attemptApproachFamily": "v7_3_crop_probe_precision_guardrail_audit",
                "successCriteria": [
                    "verify bounded v7.3 best.pt through crop-probe confidence sweep",
                    "prove precision on hard negatives, canaries, and old top-left artifact family",
                    "advance only to non-promotion pipeline eval when precision/recal gates pass",
                ],
                "failureAdaptation": "If precision or recall fails, route to the specific next debug/data family.",
            },
            {
                "attemptNumber": 2,
                "attemptApproachFamily": "v7_3_guardrail_operating_point_repair",
                "successCriteria": [
                    "adjust only audit operating-point selection from existing sweep evidence",
                    "do not retrain, relabel, promote, or mutate runtime defaults",
                ],
                "failureAdaptation": "If no safe operating point exists, write blocker truth.",
            },
            {
                "attemptNumber": 3,
                "attemptApproachFamily": "v7_3_guardrail_blocker_summary",
                "successCriteria": ["write one primary blocker", "select exactly one next family"],
                "failureAdaptation": "Stop before full-pipeline eval if guardrails are not clean.",
            },
        ],
    }
...
def _classify(summary: dict[str, Any]) -> tuple[str | None, str, bool, str, str | None]:
    if not summary["checkpointContractPassed"]:
        return (
            ...
            NEXT_CONFIG_DEBUG,
            False,
            "No verified local v7.3 bounded best.pt exists; refusing remote, stale, or base-model inference.",
            None,
        )
    if summary["missingArtifactCount"] > 0:
        return BLOCKER_ARTIFACTS, NEXT_CONFIG_DEBUG, False, "v7.3 guardrail visual/audit artifacts are missing.", None
    if summary["boundedTrainPositiveLocalizationHitRate"] < 0.90:
        return (
            BLOCKER_POSITIVE_REGRESSION,
            NEXT_POSITIVE_DIVERSITY,
            False,
            "v7.3 crop probe regressed on training positive localization.",
            None,
        )
    if summary["boundedValPositiveLocalizationHitRate"] < 0.50:
        return (
            ...
            NEXT_POSITIVE_DIVERSITY,
            False,
            "v7.3 crop probe validation recall is below the minimum guardrail.",
            None,
        )
    if summary["oldTopLeftArtifactFalsePositiveFrameRate"] > 0.05 or summary["topLeftArtifactShare"] > 0.0:
        return (
            ...
            NEXT_ARTIFACT_DEBUG,
            False,
            "The old top-left artifact family reappeared in the v7.3 guardrail audit.",
            None,
        )
    if summary["boundedTrainHardNegativeFalsePositiveFrameRate"] > 0.05 or summary["boundedValHardNegativeFalsePositiveFrameRate"] > 0.05:
        return (
            ...
            NEXT_HARD_NEGATIVE,
            False,
            "v7.3 crop probe fires on too many hard-negative crops.",
            None,
        )
    if summary["heldoutCanaryFalsePositiveFrameRate"] > 0.10:
        return (
            ...
            NEXT_HARD_NEGATIVE,
            False,
            "v7.3 crop probe floods heldout hard-negative canaries.",
            None,
        )
    ...
    if summary["giantBoxShare"] > 0.0 or (area_ratio is not None and (area_ratio < 0.25 or area_ratio > 4.0)):
        return (
            ...
            NEXT_CONFIG_DEBUG,
            False,
            "v7.3 crop probe produced badly scaled boxes relative to reviewed ball labels.",
            None,
        )
    if summary["nearConstantLowConfidenceFlood"]:
        return (
            ...
            NEXT_CONFIDENCE,
            False,
            "Near-constant low-confidence flood signature reappeared in v7.3.",
            None,
        )
    secondary = "v7_3_validation_positive_recall_limited" if summary["boundedValPositiveLocalizationHitRate"] < 0.70 else None
    return (
        None,
        NEXT_PIPELINE,
        True,
        "v7.3 bounded crop detector passed precision guardrails. Advance to full-pipeline non-promotion evaluation; do not promote.",
        secondary,
    )
...
def _markdown_summary(summary: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# V7.3 Crop Probe Precision Guardrail Audit",
            "",
...
def run_v7_3_crop_probe_precision_guardrail_audit(
    *,
    ...
    attempt_approach_family: str = "v7_3_crop_probe_precision_guardrail_audit",
) -> dict[str, Any]:
    ...
    bounded_summary = guardrail_v7_1._load_json(bounded_root / "v7_3_bounded_retrain_summary.json", required=True)
    ...
    if primary_blocker is None and miss_analysis["missCount"] > 0 and summary["boundedValPositiveLocalizationHitRate"] < 0.70:
        secondary = secondary or "v7_3_validation_positive_recall_limited"
    summary.update(
        {
    ...
    _write_json(output_root / "v7_3_crop_probe_precision_guardrail_summary.json", summary)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the v7.3 crop-probe precision guardrail audit.")
    parser.add_argument("--storage-root", type=Path, default=DEFAULT_STORAGE_ROOT)
    parser.add_argument("--candidate-name", default=DEFAULT_CANDIDATE_NAME)
    parser.add_argument("--attempt-number", type=int, default=1)
    parser.add_argument("--attempt-approach-family", default="v7_3_crop_probe_precision_guardrail_audit")
    args = parser.parse_args()
    payload = run_v7_3_crop_probe_precision_guardrail_audit(
