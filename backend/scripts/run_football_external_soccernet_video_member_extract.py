...
DEFAULT_APPROVAL_DIR_NAME = "football_external_soccernet_video_member_extract_approval_v1"
DEFAULT_RANGE_INDEX_DIR_NAME = "football_external_soccernet_split_archive_range_index_probe_v1"
DEFAULT_OUTPUT_DIR_NAME = "football_external_soccernet_video_member_extract_v1"

BLOCKER_APPROVAL_MISSING = "football_external_soccernet_video_member_extract_approval_missing"
BLOCKER_CREDENTIAL_MISSING = "football_external_soccernet_video_member_extract_credential_missing"
BLOCKER_DEPENDENCY_MISSING = "football_external_soccernet_video_member_extract_dependency_missing"
BLOCKER_EXTRACT_FAILED = "football_external_soccernet_video_member_extract_failed"

NEXT_APPROVAL = "football_external_soccernet_video_member_extract_approval"
NEXT_SECRET_SETUP = "football_external_soccernet_secret_env_setup"
NEXT_DEPENDENCY_SETUP = "football_external_soccernet_zip_member_extract_dependency_setup"
NEXT_EXTRACT_DEBUG = "football_external_soccernet_video_member_extract_debug"
...
def _attempt_plan() -> list[dict[str, Any]]:
    return [
        {
            "attemptNumber": 1,
            "attemptApproachFamily": "scoped_encrypted_video_member_extract",
            "successCriteria": [
                "fetch and decrypt only the approved encrypted 224p video member",
                "do not fetch the full archive or 720p member",
                "persist credential-free extraction and range audit artifacts",
            ],
            "failureAdaptation": "If extraction fails, repair runtime credential/dependency/range contract.",
        },
        {
            "attemptNumber": 2,
            "attemptApproachFamily": "video_member_extract_runtime_repair",
            "successCriteria": [
                "repair pyzipper dependency, sparse ZIP assembly, or runtime credential plumbing",
                "preserve scoped member extraction only",
            ],
            "failureAdaptation": "If extraction still fails, write blocker truth.",
        },
        {
            "attemptNumber": 3,
            "attemptApproachFamily": "video_member_extract_blocker_summary",
            "successCriteria": [
                "select exactly one next family",
                "stop before frame probing if no playable MP4 is produced",
            ],
            "failureAdaptation": "Route to approval, secret setup, dependency setup, or extract debug.",
        },
    ]
...
def _load_inputs(storage_root: Path, candidate_name: str) -> dict[str, Any]:
    ...
    return {
        ...
        "approvalSummary": _load_json(approval_root / "video_member_extract_approval_summary.json"),
        "extractContract": _load_json(approval_root / "video_member_extract_approval_contract.json"),
...
def _approval_ready(summary: dict[str, Any] | None, contract: dict[str, Any] | None) -> bool:
    return bool(
        ...
        and summary.get("videoMemberExtractionApproved") is True
        and summary.get("fullArchiveDownloadApproved") is False
        and summary.get("videoMemberExtractionExecuted") is False
        and summary.get("trainingExecuted") is False
        and summary.get("runtimeDefaultMutationExecuted") is False
        and isinstance(contract, dict)
        and contract.get("contractName") == "football_external_soccernet_video_member_extract"
        and contract.get("videoMemberExtractionApproved") is True
        and contract.get("fullArchiveDownloadApproved") is False
        and bool(contract.get("approvedVideoMemberPath"))
    )


def _credential_audit(env: Mapping[str, str], contract: dict[str, Any] | None, injected: bool) -> dict[str, Any]:
    required = bool((contract or {}).get("requiresRuntimeCredential"))
...
def _range_fetch(url: str, start: int, end: int, timeout_seconds: int) -> tuple[bytes, dict[str, Any]]:
    request = Request(
        url,
        headers={
            "User-Agent": "fotball-analyst-soccernet-video-member-extract/1.0",
...
def _extract_with_pyzipper(
    *,
    ...
    result = extract_zip_members_with_pyzipper(
        python_executable=python_executable,
        sparse_zip_path=sparse_zip_path,
        member_sizes={member_path: expected_size_bytes},
        output_dir=output_dir,
        env=env,
        credential_env_var=credential_env_var,
    )
    parsed = result.get("parsedResult")
    if isinstance(parsed, dict) and isinstance(parsed.get("results"), list) and parsed["results"]:
        result["parsedResult"] = parsed["results"][0]
    return result
...
def _real_extract(
    *,
    ...
    if not url:
        return [], dependency, {"extractFailure": "No resolve URL mapping for selected archive", "rangeFetches": []}
    member_path = str(contract.get("approvedVideoMemberPath") or "")
    all_entries = [row for row in (central_directory_audit.get("zipEntries") or []) if isinstance(row, dict)]
    member = next((row for row in all_entries if str(row.get("path") or "") == member_path), None)
    if not isinstance(member, dict):
        return [], dependency, {"extractFailure": "approved video member missing from ZIP index", "rangeFetches": []}
    expected_size = int(contract.get("approvedUncompressedSizeBytes") or 0)
    indexed_size = int(member.get("uncompressedSizeBytes") or 0)
    if expected_size <= 0 or indexed_size != expected_size:
        return [], dependency, {"extractFailure": "approved video member size does not match ZIP index", "rangeFetches": []}
    end_offsets = _member_end_offsets(all_entries, central_offset)
    member_start = int(member.get("localHeaderOffset") or contract.get("approvedLocalHeaderOffset") or 0)
    member_end = int(end_offsets.get(member_path, central_offset - 1))
    central_end = archive_size - 1
    range_fetches: list[dict[str, Any]] = []
    output_dir = output_root / "extracted_video"
    with tempfile.TemporaryDirectory() as temp_dir:
        sparse_zip = Path(temp_dir) / "valid_sparse_video.zip"
        with sparse_zip.open("wb") as handle:
            handle.truncate(archive_size)
            data, audit = _range_fetch(url, member_start, member_end, timeout_seconds)
            range_fetches.append({**audit, "memberPath": member_path, "rangeKind": "video_member_local_header_and_payload"})
    ...
    return [parsed] if isinstance(parsed, dict) else [], dependency, {"extractFailure": None, "rangeFetches": range_fetches, "pyzipperExtraction": extraction}


def _write_injected_payload(output_root: Path, member_path: str, payload: bytes) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    output_dir = output_root / "extracted_video"
    out = output_dir / member_path
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_bytes(payload)
    return [{"memberPath": member_path, "relativePath": str(out.relative_to(output_dir)), "sizeBytes": len(payload)}], {"extractFailure": None, "rangeFetches": [], "injectedPayload": True}
            ...
            files.append(
                {
                    ...
                    "fileSignatureHex": prefix[:16].hex(),
                    "mp4FtypPresent": b"ftyp" in prefix,
                }
            )
    return {
        "extractedVideoFileCount": len(files),
        "extractedVideoFiles": files,
    ...
    *,
    ...
    if not approval_ready:
        return (
            ...
            NEXT_APPROVAL,
            False,
            "Video-member extraction approval truth is missing or unsafe; rerun approval before extraction.",
        )
    if credential_audit.get("credentialRequired") and credential_audit.get("credentialRuntimeAvailable") is not True:
        return (
            ...
            NEXT_SECRET_SETUP,
            False,
            "Encrypted SoccerNet video member extraction requires a runtime-only credential.",
        )
    if dependency_audit.get("dependencyReady") is False:
        return (
    ...
    files = inventory.get("extractedVideoFiles") if isinstance(inventory.get("extractedVideoFiles"), list) else []
    first = files[0] if files else {}
    if extract_audit.get("extractFailure") or int(inventory.get("extractedVideoFileCount") or 0) <= 0 or first.get("mp4FtypPresent") is not True:
        return (
            BLOCKER_EXTRACT_FAILED,
            NEXT_EXTRACT_DEBUG,
            False,
            "Approved video member extraction did not produce an MP4-looking file.",
        )
    return (
        None,
        NEXT_FRAME_PROBE,
        True,
        "Extracted the approved SoccerNet 224p video member only. Full archive and 720p member were not downloaded.",
    )
...
def _decision_matrix(primary_blocker: str | None, next_lever: str, goal_achieved: bool) -> dict[str, Any]:
    return {
        ...
        "decisions": [
            ...
            {"condition": "video_member_extract_failed", "selected": primary_blocker == BLOCKER_EXTRACT_FAILED, "primaryBlocker": BLOCKER_EXTRACT_FAILED, "nextRecommendedNextLever": NEXT_EXTRACT_DEBUG},
            {"condition": "video_frame_probe_ready", "selected": goal_achieved, "primaryBlocker": None, "nextRecommendedNextLever": NEXT_FRAME_PROBE},
        ],
    }
...
def _markdown_summary(summary: dict[str, Any]) -> str:
    return "\n".join(
        [
            ...
            f"- Extracted video files: `{summary.get('extractedVideoFileCount')}`",
...
def run_football_external_soccernet_video_member_extract(
    *,
    storage_root: Path = DEFAULT_STORAGE_ROOT,
    candidate_name: str = DEFAULT_CANDIDATE_NAME,
    output_dir_name: str = DEFAULT_OUTPUT_DIR_NAME,
    video_member_payload: bytes | None = None,
    env: Mapping[str, str] | None = None,
    timeout_seconds: int = 180,
    attempt_number: int = 1,
    attempt_approach_family: str = "scoped_encrypted_video_member_extract",
) -> dict[str, Any]:
    ...
    injected = video_member_payload is not None
    credential = _credential_audit(env, inputs["extractContract"], injected)
    dependency: dict[str, Any] = {"dependencyReady": True, "injectedPayload": injected}
    extract_audit: dict[str, Any] = {"extractFailure": None, "rangeFetches": []}
    extraction_results: list[dict[str, Any]] = []
    member_path = str((inputs["extractContract"] or {}).get("approvedVideoMemberPath") or "")

    if approval_ready and (not credential.get("credentialRequired") or credential.get("credentialRuntimeAvailable")):
        if injected:
            extraction_results, extract_audit = _write_injected_payload(output_root, member_path, video_member_payload or b"")
        else:
    ...
    summary: dict[str, Any] = {
        ...
        "approvedVideoMemberPath": member_path,
        "extractedVideoFileCount": inventory.get("extractedVideoFileCount"),
        "credentialRuntimeAvailable": credential.get("credentialRuntimeAvailable"),
        "credentialPersisted": False,
        "archiveDownloadExecuted": False,
        "fullArchiveDownloadApproved": False,
        "videoMemberExtractionExecuted": goal_achieved,
        "videoMemberDownloadExecuted": goal_achieved,
        "videoMemberFullDownloadExecuted": goal_achieved,
    ...
    batch_outcome = {
        ...
        "videoMemberRangeFetchAudit": extract_audit,
        "extractedVideoMemberInventory": inventory,
        "decisionMatrix": decision_matrix,
        "failsafeAttemptPlan": {"attemptBudget": 3, "attempts": attempts},
    }

    _write_json(output_root / "video_member_extract_summary.json", summary)
    _write_json(output_root / "credential_runtime_audit.json", credential)
    _write_json(output_root / "dependency_audit.json", dependency)
    _write_json(output_root / "video_member_range_fetch_audit.json", extract_audit)
    _write_json(output_root / "extracted_video_member_inventory.json", inventory)
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--attempt-approach-family", default="scoped_encrypted_video_member_extract")
    return parser


def main(argv: list[str] | None = None) -> int:
    args = _build_parser().parse_args(argv)
    payload = run_football_external_soccernet_video_member_extract(
