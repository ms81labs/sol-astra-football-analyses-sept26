...
DEFAULT_OUTPUT_ROOT = DEFAULT_SUITE_ROOT / "manual_review_expansion_resolution_v1"
DEFAULT_EXPANSION_ROOT = DEFAULT_SUITE_ROOT / "manual_review_expansion_v1"
...
DEFAULT_BATCH_NAME = "manual_review_expansion_resolution_v1"
...
MIN_POSITIVES_FOR_MICRO_VALIDATION = 5
...
def build_expanded_reviewed_truth_seed(
    *,
            ...
            positive_rows.append(
                {
                    ...
                    "truthUse": "expanded_reviewed_positive",
                }
            )
...
def build_decision_matrix(matrix: dict[str, Any]) -> dict[str, Any]:
    ...
    if invalid > 0:
        return {
            ...
            "successfulApproach": "B_expansion_review_overlay_repair_guidance",
            "weakEvidenceReasons": ["invalid_review_decisions_present"],
            "rationale": "Expanded review decisions contain invalid values or missing required boxes.",
        }
    if pending > 0:
        return {
            "goalAchieved": False,
            ...
            "successfulApproach": "A_expansion_review_decision_validation",
            "weakEvidenceReasons": ["pending_review_items_remaining"],
            "rationale": "Expanded review decisions are still pending; do not advance to detector-side work.",
        }
    if positives >= MIN_POSITIVES_FOR_MICRO_VALIDATION:
        return {
            "goalAchieved": True,
            "roadmapAdvanceAllowed": True,
            "batchStatus": "review_resolved",
            "nextCorrectiveFamily": "reviewed_positive_micro_validation",
            "successfulApproach": "C_expanded_reviewed_truth_seed_generation",
            "weakEvidenceReasons": [],
            "rationale": "Expanded review produced enough positive seed frames for micro-validation.",
        }
    if positives > 0:
        return {
            "goalAchieved": True,
            ...
            "successfulApproach": "C_expanded_reviewed_truth_seed_generation",
            "weakEvidenceReasons": ["reviewed_positive_seed_count_below_micro_validation_floor"],
            "rationale": "Expanded review resolved but positive evidence remains too sparse.",
        }
    if negatives > 0:
        return {
            "goalAchieved": True,
            ...
            "successfulApproach": "C_expanded_reviewed_truth_seed_generation",
            "weakEvidenceReasons": [],
            "rationale": "Expanded review refuted all positive anchors.",
        }
    return {
        "goalAchieved": False,
        "roadmapAdvanceAllowed": False,
        ...
        "rationale": "Expanded review has no actionable decisions.",
    }
...
def _markdown_summary(summary: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# Manual Review Expansion Resolution",
            "",
...
def run_promoted_v6_manual_review_expansion_resolution(
    *,
    ...
    frame_manifest = _load_optional_json_dict(expansion_root / "review_frame_manifest.json")
    refutation_guard = _load_optional_json_dict(expansion_root / "refutation_guard_manifest.json")
    ...
    truth_seed = build_expanded_reviewed_truth_seed(overlay=overlay, matrix=matrix)
    ...
    summary = {
        ...
        "attemptNumber": 1,
        "attemptBudget": 3,
        "attemptApproachFamily": "expansion_review_decision_validation",
        ...
        "extractedImageCount": frame_manifest.get("extractedImageCount"),
        "rejectedRefutationSeedCount": refutation_guard.get("rejectedSeedCount"),
    ...
    batch_outcome = {
        ...
        "sourceManifestMutated": False,
        "artifacts": {
            "manualReviewExpansionResolutionSummaryPath": str(
                output_root / "manual_review_expansion_resolution_summary.json"
            ),
            "reviewDecisionMatrixPath": str(output_root / "review_decision_matrix.json"),
            "expandedReviewedTruthSeedPath": str(output_root / "expanded_reviewed_truth_seed.json"),
        },
    }

    _write_json(output_root / "manual_review_expansion_resolution_summary.json", summary)
    _write_json(output_root / "review_decision_matrix.json", matrix)
    _write_json(output_root / "expanded_reviewed_truth_seed.json", truth_seed)
    ...
    if not decision["goalAchieved"]:
        _write_json(
            ...
            {
                **summary,
                "blockerSummaryType": "manual_review_expansion_resolution_blocked",
            },
        )
    ...
    return {
        ...
        "expandedReviewedTruthSeed": truth_seed,
...
def main() -> None:
    args = parse_args()
    payload = run_promoted_v6_manual_review_expansion_resolution(
