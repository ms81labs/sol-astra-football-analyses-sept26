...
from backend.app.pilot_tracking import (
    TRACKEVAL_COMMIT as TRACKEVAL_COMMIT,
    evaluation_frame_ids as evaluation_frame_ids,
    _iou as _iou,
    _prediction_bbox as _prediction_bbox,
    _prediction_kind as _prediction_kind,
    build_trackeval_sequence_data as build_trackeval_sequence_data,
    _longest_tracking_gap as _longest_tracking_gap,
    evaluate_tracking as evaluate_tracking,
)
...
def _transition_errors(
    ...
    for direction in (("home", "away"), ("away", "home")):
        ...
        truth_indices, prediction_indices = linear_sum_assignment(costs)
        errors.extend((truth_times[truth_index], prediction_times[prediction_index] - truth_times[truth_index]) for truth_index, prediction_index in zip(truth_indices, prediction_indices, strict=False))
...
def _match_events(
    ...
    *,
    ...
    truth_indices, prediction_indices = linear_sum_assignment(costs)
    return sum(prediction_index < len(predictions) and costs[truth_index, prediction_index] < unmatched_cost for truth_index, prediction_index in zip(truth_indices, prediction_indices, strict=False))
