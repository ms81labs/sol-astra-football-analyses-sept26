...
import backend.scripts.run_v7_1_crop_probe_precision_guardrail_audit as guardrail
...
def _write_bounded_bundle(tmp_path: Path, *, local_checkpoint: bool = True) -> Path:
    ...
    (bounded_root / "v7_1_bounded_retrain_summary.json").parent.mkdir(parents=True, exist_ok=True)
    (bounded_root / "v7_1_bounded_retrain_summary.json").write_text(
        json.dumps(
            {
                "batchName": "v7_1_bounded_retrain",
                "goalAchieved": True,
            ...
            for index, row in enumerate(rows):
                if row["truthType"] == "positive" and conf <= 0.10:
                    if mode == "val_fail" and set_name == "bounded_val_positive":
                        pred = None
                    elif mode == "limited_val" and set_name == "bounded_val_positive" and index % 2 == 1:
                        pred = None
                    else:
                    ...
                    predictions[set_name].append(
                        {
                            ...
                            "confidence": 0.42 if pred else None,
                            "classId": 0 if pred else None,
                            "predCropBbox": pred,
                        }
                    )
                    continue
                if mode == "top_left" and row["truthType"] == "negative" and conf <= 0.10:
                    predictions[set_name].append(
                        {
                            "exampleId": row["exampleId"],
                            "confidence": 0.19,
                            "classId": 0,
                            "predCropBbox": [0.0, 0.0, 220.0, 220.0],
                        }
                    )
                    continue
                predictions[set_name].append(
...
def test_crop_probe_guardrail_passes_with_clean_precision_and_limited_recall(tmp_path: Path) -> None:
    candidate_root = _write_bounded_bundle(tmp_path)

    payload = guardrail.run_v7_1_crop_probe_precision_guardrail_audit(
        storage_root=tmp_path,
        predictor=_predictor(mode="limited_val"),
    )

    output_root = candidate_root / "v7_1_crop_probe_precision_guardrail_audit_v1"
    summary = json.loads((output_root / "v7_1_crop_probe_precision_guardrail_summary.json").read_text(encoding="utf-8"))

    assert payload["primaryBlocker"] is None
    assert payload["checkpointContractPassed"] is True
    assert payload["trainingExecuted"] is False
    assert payload["boundedTrainPositiveLocalizationHitRate"] == 1.0
    assert payload["boundedValPositiveLocalizationHitRate"] == 0.5
    assert payload["heldoutCanaryFalsePositiveFrameRate"] == 0.0
    assert payload["oldTopLeftArtifactFalsePositiveFrameRate"] == 0.0
    assert payload["secondaryConcern"] == "v7_1_validation_positive_recall_limited"
    assert payload["nextRecommendedNextLever"] == "v7_1_full_pipeline_non_promotion_eval"
...
def test_crop_probe_guardrail_fails_closed_without_local_checkpoint(tmp_path: Path) -> None:
    _write_bounded_bundle(tmp_path, local_checkpoint=False)
    ...
    payload = guardrail.run_v7_1_crop_probe_precision_guardrail_audit(storage_root=tmp_path, predictor=predictor)

    assert predictor_calls == []
    assert payload["primaryBlocker"] == "v7_1_crop_probe_checkpoint_contract_failure"
...
def test_crop_probe_guardrail_blocks_top_left_artifact_regression(tmp_path: Path) -> None:
    _write_bounded_bundle(tmp_path)
    ...
    assert payload["primaryBlocker"] == "v7_1_crop_probe_top_left_artifact_regression"
    assert payload["topLeftArtifactShare"] > 0.0
    assert payload["nextRecommendedNextLever"] == "v7_1_artifact_regression_debug"


def test_crop_probe_guardrail_routes_hard_validation_recall_failure_to_diversity(tmp_path: Path) -> None:
    _write_bounded_bundle(tmp_path)
    ...
    assert payload["primaryBlocker"] == "v7_1_crop_probe_validation_recall_insufficient"
    assert payload["boundedTrainPositiveLocalizationHitRate"] == 1.0
