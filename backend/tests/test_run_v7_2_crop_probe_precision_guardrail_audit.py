...
import backend.scripts.run_v7_2_crop_probe_precision_guardrail_audit as guardrail
...
def _write_v7_2_bounded_bundle(tmp_path: Path, *, local_checkpoint: bool = True) -> Path:
    ...
    if local_checkpoint:
        ...
        best.write_bytes(b"v7-2-best")
        last.write_bytes(b"v7-2-last")
    (bounded_root / "v7_2_bounded_retrain_summary.json").write_text(
        json.dumps(
            {
                ...
                "trainingDatasetPositiveCount": 414,
...
def test_v7_2_crop_probe_guardrail_passes_strong_precision_and_recall(tmp_path: Path) -> None:
    candidate_root = _write_v7_2_bounded_bundle(tmp_path)

    payload = guardrail.run_v7_2_crop_probe_precision_guardrail_audit(
        storage_root=tmp_path,
        predictor=_predictor(),
    )

    output_root = candidate_root / "v7_2_crop_probe_precision_guardrail_audit_v1"
    summary = json.loads((output_root / "v7_2_crop_probe_precision_guardrail_summary.json").read_text(encoding="utf-8"))

    assert payload["batchName"] == "v7_2_crop_probe_precision_guardrail_audit"
    ...
    assert payload["nextRecommendedNextLever"] == "v7_2_full_pipeline_non_promotion_eval"
...
def test_v7_2_crop_probe_guardrail_fails_closed_without_local_checkpoint(tmp_path: Path) -> None:
    _write_v7_2_bounded_bundle(tmp_path, local_checkpoint=False)
    ...
    payload = guardrail.run_v7_2_crop_probe_precision_guardrail_audit(storage_root=tmp_path, predictor=predictor)

    assert predictor_calls == []
    assert payload["primaryBlocker"] == "v7_2_crop_probe_checkpoint_contract_failure"
...
def test_v7_2_crop_probe_guardrail_blocks_top_left_artifact_regression(tmp_path: Path) -> None:
    _write_v7_2_bounded_bundle(tmp_path)
    ...
    assert payload["primaryBlocker"] == "v7_2_crop_probe_top_left_artifact_regression"
    assert payload["topLeftArtifactShare"] > 0.0
    assert payload["nextRecommendedNextLever"] == "v7_2_artifact_regression_debug"


def test_v7_2_crop_probe_guardrail_routes_hard_validation_recall_failure_to_diversity(tmp_path: Path) -> None:
    _write_v7_2_bounded_bundle(tmp_path)
    ...
    assert payload["primaryBlocker"] == "v7_2_crop_probe_validation_recall_insufficient"
    assert payload["boundedTrainPositiveLocalizationHitRate"] == 1.0
    assert payload["boundedValPositiveLocalizationHitRate"] == 0.0
    assert payload["nextRecommendedNextLever"] == "v7_2_positive_diversity_refresh"
