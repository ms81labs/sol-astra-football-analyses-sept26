...
import backend.scripts.run_v7_2_post_runtime_default_source_robustness_validation as post_validation
...
def _write_post_default_inputs(
    ...
    *,
    ...
    change_root = suite_root / "v7_2_runtime_default_change_validation_v1"
    ...
    registry = {
        ...
        "runtimeDefaultProfileName": profile_name if runtime_mutated else None,
    ...
    if include_change_summary:
        ...
        _write_json(
            change_root / "runtime_default_source_robustness_regeneration_audit.json",
            {
                ...
                "checks": {
                    "allNearViableDeficitsCovered": source_gate_passed,
                    "allViableDeficitsCovered": source_gate_passed,
                    "allNearViableEdgeShareClears": source_gate_passed,
                    "allViableEdgeShareClears": source_gate_passed,
                },
            },
        )
        _write_json(
    ...
    # Historical pre-mutation suite truth may still carry the old blocker. The post-default
    # validator must not treat this stale surface as the active runtime truth.
    _write_json(
...
def test_post_runtime_default_validation_passes_from_active_registry(tmp_path: Path) -> None:
    ...
    payload = post_validation.run_v7_2_post_runtime_default_source_robustness_validation(storage_root=tmp_path)

    registry = json.loads(paths["registryPath"].read_text(encoding="utf-8"))
    output_root = paths["suiteRoot"] / "v7_2_post_runtime_default_source_robustness_validation_v1"
    ...
    assert registry["nextRecommendedNextLever"] == "v7_2_runtime_default_rollout_closeout"
...
def test_old_suite_summary_blocker_is_treated_as_historical_not_active(tmp_path: Path) -> None:
...
def test_missing_runtime_default_change_truth_fails_closed(tmp_path: Path) -> None:
    ...
    assert payload["primaryBlocker"] == "v7_2_post_default_source_truth_missing"
    assert payload["postRuntimeDefaultSourceRobustnessValidated"] is False
    assert payload["nextRecommendedNextLever"] == "v7_2_post_default_source_robustness_regeneration"
    assert paths["registryPath"].read_text(encoding="utf-8") == before


def test_stale_runtime_registry_fails_closed(tmp_path: Path) -> None:
    paths = _write_post_default_inputs(tmp_path, registry_version="v7.1")
    before = paths["registryPath"].read_text(encoding="utf-8")

    payload = post_validation.run_v7_2_post_runtime_default_source_robustness_validation(storage_root=tmp_path)

    assert payload["primaryBlocker"] == "v7_2_post_default_registry_contract_gap"
    assert payload["nextRecommendedNextLever"] == "v7_2_post_default_registry_contract_repair"
    assert payload["postRuntimeDefaultSourceRobustnessValidated"] is False
    assert paths["registryPath"].read_text(encoding="utf-8") == before
...
def test_failing_source_blocker_resurrection_fails_closed(tmp_path: Path) -> None:
    ...
    assert payload["primaryBlocker"] == "v7_2_post_default_failing_source_not_viable_resurrected"
    assert payload["failingSourceNotViableBlockerPresent"] is True
    assert payload["nextRecommendedNextLever"] == "v7_2_post_default_source_robustness_regression_debug"
    assert paths["registryPath"].read_text(encoding="utf-8") == before
