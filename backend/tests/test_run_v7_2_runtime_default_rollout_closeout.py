...
import backend.scripts.run_v7_2_runtime_default_rollout_closeout as closeout
...
def _write_closeout_inputs(
    ...
    *,
    ...
    registry_next: str = "v7_2_runtime_default_rollout_closeout",
    ...
    profile = "source_robustness_shadow_v7_2_default_path_inboard_recovery_v1"

    _write_json(
        suite_root / "v7_2_default_path_inboard_ball_recovery_v1" / "inboard_ball_recovery_summary.json",
        {
    ...
    _write_json(
        suite_root / "v7_2_runtime_default_change_validation_v1" / "runtime_default_change_summary.json",
        {
            ...
            "runtimeDefaultProfileName": profile,
    ...
    _write_json(
        automation_path,
        {
            "activeBatchName": "v7_2_post_runtime_default_source_robustness_validation",
            "queueItemName": "v7_2_post_runtime_default_source_robustness_validation",
            "currentStep": "selected v7_2_runtime_default_rollout_closeout"
            if unattended_mentions_closeout
            else "still waiting on post validation",
            "nextExpectedAction": "Run v7_2_runtime_default_rollout_closeout"
            if unattended_mentions_closeout
            else "Run v7_2_post_runtime_default_source_robustness_validation",
            "lastVerificationSummary": {
                "v7_2_post_runtime_default_source_robustness_validation": "next=v7_2_runtime_default_rollout_closeout"
                if unattended_mentions_closeout
                else "next=v7_2_post_runtime_default_source_robustness_validation"
            },
        },
    )
...
def test_rollout_closeout_passes_and_marks_registry_completed(tmp_path: Path) -> None:
    ...
    payload = closeout.run_v7_2_runtime_default_rollout_closeout(storage_root=tmp_path)

    registry = json.loads(paths["runtimePath"].read_text(encoding="utf-8"))
    output_root = paths["suiteRoot"] / "v7_2_runtime_default_rollout_closeout_v1"
    ...
    assert registry["nextRecommendedNextLever"] == "football_external_dataset_access_review"
...
def test_historical_suite_blocker_is_archived_not_active(tmp_path: Path) -> None:
...
def test_missing_post_default_validation_fails_closed(tmp_path: Path) -> None:
    ...
    assert payload["primaryBlocker"] == "v7_2_rollout_closeout_post_validation_missing"
...
def test_registry_not_pointing_to_closeout_fails_closed(tmp_path: Path) -> None:
    paths = _write_closeout_inputs(tmp_path, registry_next="v7_2_post_runtime_default_source_robustness_validation")
    before = paths["runtimePath"].read_text(encoding="utf-8")

    payload = closeout.run_v7_2_runtime_default_rollout_closeout(storage_root=tmp_path)

    assert payload["primaryBlocker"] == "v7_2_rollout_closeout_registry_contract_drift"
    assert payload["nextRecommendedNextLever"] == "v7_2_runtime_default_rollout_contract_repair"
    assert paths["runtimePath"].read_text(encoding="utf-8") == before
...
def test_unattended_status_drift_routes_to_contract_repair(tmp_path: Path) -> None:
    ...
    assert payload["primaryBlocker"] == "v7_2_rollout_closeout_unattended_status_drift"
    assert payload["nextRecommendedNextLever"] == "v7_2_runtime_default_rollout_contract_repair"
    assert paths["runtimePath"].read_text(encoding="utf-8") == before
