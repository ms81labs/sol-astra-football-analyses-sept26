...
import backend.scripts.run_football_external_soccertrack_analysis_product_ui_route_implementation as route_batch
...
def _write_ui_binding(storage_root: Path, *, ready: bool = True) -> Path:
    binding_root = _candidate_root(storage_root) / "football_external_soccertrack_analysis_product_ui_binding_v1"
    _write_json(
        binding_root / "analysis_product_ui_binding_summary.json",
        {
            "batchName": "football_external_soccertrack_analysis_product_ui_binding",
            "goalAchieved": ready,
            "primaryBlocker": None if ready else "football_external_soccertrack_analysis_product_ui_contract_gap",
    ...
    _write_json(
        binding_root / "analysis_product_ui_route_contract.json",
        {
            ...
            "routePath": "/external/soccertrack/117092/analysis",
    ...
    (binding_root / "analysis_product_ui_render_smoke.html").write_text(
        "<!doctype html><html><body>SoccerTrack 117092 external fixture Events not detector evaluation</body></html>",
        encoding="utf-8",
    )
    return binding_root


def test_soccertrack_ui_route_implementation_smokes_live_routes_and_writes_artifacts(tmp_path: Path) -> None:
    _write_ui_binding(tmp_path)

    payload = route_batch.run_football_external_soccertrack_analysis_product_ui_route_implementation(
        storage_root=tmp_path
    )

    output_root = _candidate_root(tmp_path) / "football_external_soccertrack_analysis_product_ui_route_implementation_v1"
    ...
    assert response_fixture["body"]["hero"]["title"] == "SoccerTrack 117092 external fixture"


def test_soccertrack_ui_route_implementation_blocks_without_ui_binding(tmp_path: Path) -> None:
    payload = route_batch.run_football_external_soccertrack_analysis_product_ui_route_implementation(
        storage_root=tmp_path
    )

    assert payload["goalAchieved"] is False
    assert payload["primaryBlocker"] == "football_external_soccertrack_analysis_product_ui_binding_missing"
    assert payload["nextRecommendedNextLever"] == "football_external_soccertrack_analysis_product_ui_binding"


def test_soccertrack_ui_route_implementation_contains_three_adaptive_attempts(tmp_path: Path) -> None:
    _write_ui_binding(tmp_path)
    ...
    assert payload["attemptPlanFamilies"] == [
        "soccertrack_analysis_product_ui_route_smoke",
        "soccertrack_analysis_product_ui_route_contract_repair",
        "soccertrack_analysis_product_ui_route_blocker_summary",
    ]
