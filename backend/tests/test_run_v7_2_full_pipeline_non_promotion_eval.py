...
import backend.scripts.run_v7_2_full_pipeline_non_promotion_eval as pipeline_eval
...
def _positive_example(frame: int, size: int, *, split: str = "train") -> dict[str, object]:
    return {
        "exampleId": f"v7-2-pos-{frame}-{size}",
        "frameIndex": frame,
...
def _nonlocal_positive_row(example: dict[str, object]) -> dict[str, object]:
    return {
        "exampleId": example["exampleId"],
        "truthType": "positive",
        "split": "train" if example["split"] == "train" else "val",
        "gtCropBbox": example["cropFrameBbox"],
        "predCropBbox": [20.0, 20.0, 36.0, 36.0],
        "confidence": 0.18,
        "iou": 0.0,
        "centerDistancePx": 140.0,
        "isLocalizationHit": False,
        "isFalsePositive": False,
        "isTopLeftArtifact": False,
        "isGiantBox": False,
        "detectedBoxAreaToGtBoxAreaRatio": 1.0,
    }
...
def _write_guardrail_bundle(tmp_path: Path, *, local_checkpoint: bool = True, mode: str = "pass") -> Path:
    ...
    if mode == "projection_error":
        positives[1]["sourceFrameBbox"] = {"x1": 300.0, "y1": 300.0, "x2": 316.0, "y2": 316.0}
    if mode == "nonlocal_prediction":
        positives[2]["frameIndex"] = positives[1]["frameIndex"]
        positives[2]["sourceFrameBbox"] = positives[1]["sourceFrameBbox"]
    if mode == "repaired_bounds":
        positives[1] = {
            **positives[1],
            "cropBounds": [1951.322, -42.365, 2335.322, 341.635],
            "cropSizePx": 384,
            "cropFrameBbox": {"x1": 187.55745098039188, "y1": 144.38998280309545, "x2": 197.08745098039162, "y2": 154.87998280309546},
            "sourceFrameBbox": {"x1": 2138.557450980392, "y1": 144.38998280309545, "x2": 2148.0874509803916, "y2": 154.87998280309546},
        }

    _write_json(
        manifest_root / "v7_2_training_manifest.json",
        {
    ...
    if mode == "repaired_bounds":
        _write_json(
            ...
            {
                "positiveCropBoundsRepairedCount": 1,
                "rows": [
                    {
                        "exampleId": positives[1]["exampleId"],
                        "cropBoundsRepaired": True,
                        "cropFrameBbox": positives[1]["cropFrameBbox"],
                        "cropHeight": 384,
                        "cropWidth": 384,
                        "sourceFrameBbox": positives[1]["sourceFrameBbox"],
                    }
                ],
            },
        )
    ...
    val_rows = [
        _nonlocal_positive_row(positives[1]) if mode == "nonlocal_prediction" else _positive_row(positives[1], hit=mode != "coverage_gap"),
    ...
    _write_json(
        bounded_root / "v7_2_bounded_retrain_summary.json",
        {
...
def test_v7_2_full_pipeline_eval_passes_strong_pipeline_signal(tmp_path: Path) -> None:
    candidate_root = _write_guardrail_bundle(tmp_path)

    payload = pipeline_eval.run_v7_2_full_pipeline_non_promotion_eval(storage_root=tmp_path)

    output_root = candidate_root / "v7_2_full_pipeline_non_promotion_eval_v1"
    summary = json.loads((output_root / "v7_2_full_pipeline_non_promotion_summary.json").read_text(encoding="utf-8"))
...
def test_v7_2_full_pipeline_eval_fails_closed_without_checkpoint_contract(tmp_path: Path) -> None:
    _write_guardrail_bundle(tmp_path, local_checkpoint=False)

    payload = pipeline_eval.run_v7_2_full_pipeline_non_promotion_eval(storage_root=tmp_path)

    assert payload["primaryBlocker"] == "v7_2_full_pipeline_checkpoint_contract_failure"
    assert payload["checkpointContractPassed"] is False
    assert payload["nextRecommendedNextLever"] == "v7_2_training_config_or_export_debug"


def test_v7_2_full_pipeline_eval_routes_candidate_crop_coverage_gap(tmp_path: Path) -> None:
    _write_guardrail_bundle(tmp_path, mode="coverage_gap")

    payload = pipeline_eval.run_v7_2_full_pipeline_non_promotion_eval(storage_root=tmp_path)

    assert payload["primaryBlocker"] == "v7_2_full_pipeline_candidate_crop_coverage_gap"
    assert payload["candidateCropCoverageRate"] < 1.0
    assert payload["nextRecommendedNextLever"] == "v7_2_candidate_crop_generation_refresh"


def test_v7_2_full_pipeline_eval_routes_projection_error(tmp_path: Path) -> None:
    _write_guardrail_bundle(tmp_path, mode="projection_error")

    payload = pipeline_eval.run_v7_2_full_pipeline_non_promotion_eval(storage_root=tmp_path)

    assert payload["primaryBlocker"] == "v7_2_full_pipeline_crop_projection_error"
    assert payload["projectionAuditPassed"] is False
    assert payload["nextRecommendedNextLever"] == "v7_2_crop_projection_contract_fix"


def test_v7_2_full_pipeline_eval_uses_export_repaired_crop_bounds_for_projection(tmp_path: Path) -> None:
    _write_guardrail_bundle(tmp_path, mode="repaired_bounds")

    payload = pipeline_eval.run_v7_2_full_pipeline_non_promotion_eval(storage_root=tmp_path)

    assert payload["primaryBlocker"] is None
    assert payload["projectionAuditPassed"] is True
    assert payload["positiveCropBoundsRepairedCount"] == 1
    assert payload["sourceFrameLocalizationHitRate"] == 1.0


def test_v7_2_full_pipeline_eval_does_not_treat_nonlocal_crop_prediction_as_projection_error(tmp_path: Path) -> None:
    _write_guardrail_bundle(tmp_path, mode="nonlocal_prediction")
...
def test_v7_2_full_pipeline_eval_blocks_top_left_artifact_regression(tmp_path: Path) -> None:
    _write_guardrail_bundle(tmp_path, mode="top_left")

    payload = pipeline_eval.run_v7_2_full_pipeline_non_promotion_eval(storage_root=tmp_path)

    assert payload["primaryBlocker"] == "v7_2_full_pipeline_top_left_artifact_regression"
    assert payload["oldTopLeftArtifactFalsePositiveFrameRate"] == 1.0
    assert payload["nextRecommendedNextLever"] == "v7_2_artifact_regression_debug"
