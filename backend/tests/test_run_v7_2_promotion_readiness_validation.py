...
import backend.scripts.run_v7_2_promotion_readiness_validation as promotion
...
def _build_v7_2_truth(
    ...
    *,
    ...
    weights_path.write_bytes(b"v7.2 best weights")
    ...
    _write_json(candidate_root / "v7_2_export_label_overlay_audit_v1" / "v7_2_label_overlay_audit.json", export_summary)
    ...
    _write_json(candidate_root / "v7_2_bounded_retrain_v1" / "v7_2_bounded_retrain_summary.json", bounded_summary)
...
def test_clean_v7_2_truth_validates_controlled_promotion_and_registry(tmp_path: Path) -> None:
    candidate_root = _build_v7_2_truth(tmp_path)

    payload = promotion.run_v7_2_promotion_readiness_validation(storage_root=tmp_path)

    output_root = candidate_root / "v7_2_promotion_readiness_validation_v1"
    ...
    assert payload["nextRecommendedNextLever"] == "promoted_v7_2_source_robustness_validation"
    assert runtime_registry["trainingCandidateVersion"] == "v7.2"
...
def test_runtime_default_mutation_can_be_allowed_without_executing_it(tmp_path: Path) -> None:
    _build_v7_2_truth(tmp_path, source_blockers=[])
...
def test_missing_full_pipeline_truth_fails_closed(tmp_path: Path) -> None:
    _build_v7_2_truth(tmp_path, include_pipeline=False)

    payload = promotion.run_v7_2_promotion_readiness_validation(storage_root=tmp_path)

    assert payload["promotionValidated"] is False
    assert payload["primaryBlocker"] == "v7_2_promotion_readiness_missing_upstream_truth"
...
def test_full_pipeline_flood_regression_routes_runtime_integration_fix(tmp_path: Path) -> None:
    ...
    assert payload["promotionValidated"] is False
    assert payload["primaryBlocker"] == "v7_2_promotion_full_pipeline_flood_regression"
    assert payload["nextRecommendedNextLever"] == "v7_2_runtime_integration_fix"
...
def test_positive_recall_failure_routes_positive_diversity_refresh(tmp_path: Path) -> None:
    ...
    assert payload["promotionValidated"] is False
    assert payload["primaryBlocker"] == "v7_2_promotion_positive_recall_insufficient"
