...
The production correctness/cleanup command permits **zero** diagnostics. The broader Ruff check covers `F,B,BLE,S110,B904,C901` across `backend/` and the gate itself. Mypy checks `workbench/`, `schemas.py`, `settings.py`, and `run_benchmarks.py` with the Pydantic plugin. Imported modules are analysed silently; missing third-party stubs remain `Any`. This is a bounded initial type gate, not full strict typing or a substitute for runtime tests.
