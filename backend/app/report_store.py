...
def validate_record(document, match_id: str, generation_id: str, task: str) -> None:
    ...
    _validate_record_metadata(document)
    ...
    if schema in {"report_draft_v1", "legacy_referenced_report_v0"}:
        ...
        _validate_deterministic_payload(payload, grounding, disposition)
    else:
        raise ValueError("Unsupported output schema")


def _validate_record_metadata(document) -> None:
    """Check identity and envelope metadata in their original failure order."""
...
def _validate_deterministic_payload(payload, grounding, disposition) -> None:
...
def _validate_narrative_payload(payload, match_id, generation_id, task, schema, disposition):
    ...
    """
    base = _narrative_draft_base(payload, match_id, generation_id, task, schema)
    _strip_claim_metadata(base)
...
def _narrative_draft_base(payload, match_id, generation_id, task, schema) -> dict:
    """Adapt the supported stored schema without mutating the persisted payload."""
...
def _strip_claim_metadata(base) -> None:
    """Validate publication labels before stripping server-only claim fields."""
    for name, model in (("metricClaims", MetricClaim), ("observations", ObservationClaim)):
        ...
        for item in items:
            ...
            if name == "metricClaims":
                _validate_metric_claim_coverage(item)
        base[name] = [{key: value for key, value in item.items() if key in model.model_fields} for item in items]


def _validate_metric_claim_coverage(item) -> None:
    """Check a stored metric claim's coverage and limitation labels."""
...
class ReportStore:
    ...
    def _verified_candidates(self, directory: Path, match_id: str, selected: str,
                             task: str, notices: list[_ReportNotice]) -> list[dict]:
        """Isolate invalid optional records; leave generation/lock selection to view."""
        candidates = []
        if directory.exists():
    ...
    def view(self, match_id: str, *, generation_id: str | None = None) -> dict:
        ...
        with self.storage.generation_snapshot(match_id, generation_id=generation_id) as ref:
            ...
            for task in sorted(TASKS):
                ...
                candidates = self._verified_candidates(directory, match_id, selected, task, notices)
                if candidates:
