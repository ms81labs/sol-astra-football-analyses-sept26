...
describe('App leftover HTTP surfaces', () => {
    ...
    it('posts research paths, rights evaluate, and retention delete without leftover product paths, granted permission, or authorisedPolicy true', async () => {
      stubPitchCanvas();
      vi.mocked(api.fetchMatches).mockResolvedValue([readyMatch('match-a', 'Match A')]);
      vi.mocked(api.fetchMatchWorkspace).mockResolvedValue(loadedWorkspace('match-a', 'Match A'));
      const fetchMock = vi.fn((input: RequestInfo, init?: RequestInit) => {
        ...
        if (url.endsWith('/api/research/paths') && init?.method === 'POST') {
          return Promise.resolve({
            ok: true,
            json: async () => ({ allowed: true }),
          } as Response);
        }
        if (url.endsWith('/api/rights/evaluate') && init?.method === 'POST') {
          return Promise.resolve({
            ok: true,
            json: async () => ({
              allowed: false,
        ...
        if (url.endsWith('/api/retention/delete') && init?.method === 'POST') {
          return Promise.resolve({
            ok: true,
            json: async () => ({
              kind: '',
      ...
      render(<App />);
      await screen.findByText('Match A', { selector: 'header span' });
      await waitFor(() => expect(screen.getByRole('button', { name: /request research paths/i })).toBeTruthy());
      expect(fetchMock.mock.calls.some(([url]) => String(url).includes('/api/research/paths'))).toBe(false);
      expect(fetchMock.mock.calls.some(([url]) => String(url).includes('/api/rights/evaluate'))).toBe(false);
      expect(fetchMock.mock.calls.some(([url]) => String(url).includes('/api/retention/delete'))).toBe(false);
      fireEvent.click(screen.getByRole('button', { name: /request research paths/i }));
      await waitFor(() => {
        expect(fetchMock.mock.calls.some(([url, init]) => (
          String(url).endsWith('/api/research/paths')
          && init?.method === 'POST'
          && init.body === '{}'
        ))).toBe(true);
      });
      fireEvent.click(screen.getByRole('button', { name: /request rights evaluate/i }));
      await waitFor(() => {
        expect(fetchMock.mock.calls.some(([url, init]) => (
          String(url).endsWith('/api/rights/evaluate')
          && init?.method === 'POST'
          && init.body === '{}'
        ))).toBe(true);
      });
      fireEvent.click(screen.getByRole('button', { name: /request retention delete/i }));
      await waitFor(() => {
        expect(fetchMock.mock.calls.some(([url, init]) => (
          String(url).endsWith('/api/retention/delete')
          && init?.method === 'POST'
          && init.body === '{}'
        ))).toBe(true);
      });
      expect(fetchMock.mock.calls.some(([url, init]) => (
        String(url).includes('/api/research/paths')
        && Boolean(init?.body && (String(init.body).includes('paths') || String(init.body).includes('backend/')))
      ))).toBe(false);
      expect(fetchMock.mock.calls.some(([url, init]) => (
        String(url).includes('/api/rights/evaluate')
        && Boolean(init?.body && (String(init.body).includes('commercialPermission') || String(init.body).includes('granted')))
      ))).toBe(false);
      expect(fetchMock.mock.calls.some(([url, init]) => (
        String(url).includes('/api/retention/delete')
        && Boolean(init?.body && (String(init.body).includes('authorisedPolicy') || String(init.body).includes('kind')))
      ))).toBe(false);
      expect(fetchMock.mock.calls.some(([url]) => String(url).includes('/api/quota'))).toBe(false);
      expect(fetchMock.mock.calls.some(([url]) => String(url).includes('/api/training/admit'))).toBe(false);
      expect(fetchMock.mock.calls.some(([url]) => String(url).includes('/api/receipts/promotion'))).toBe(false);
      ...
      expect(fetchMock.mock.calls.some(([url]) => String(url).includes('/api/access/object'))).toBe(false);
      expect(fetchMock.mock.calls.some(([url]) => String(url).includes('/api/ownership/hysteresis'))).toBe(false);
      expect(screen.queryByText(/empty states stay insufficient_coverage/i)).toBeNull();
      expect(screen.queryByText(/empty template stays eventCount 0/i)).toBeNull();
      expect(screen.queryByText(/track edit still rebuilds ownership/i)).toBeNull();
      const researchPaths = within(screen.getByRole('region', { name: /unforced research paths/i }));
      expect(researchPaths.getByText(/keeps allowed true/i)).toBeTruthy();
      expect(researchPaths.getByText(/client product paths are not sent/i)).toBeTruthy();
      expect(researchPaths.getByText(/empty paths stay non-product/i)).toBeTruthy();
      const rightsEvaluate = within(screen.getByRole('region', { name: /unforced rights evaluate/i }));
      expect(rightsEvaluate.getByText(/keeps allowed false/i)).toBeTruthy();
      expect(rightsEvaluate.getByText(/client commercialPermission granted is not sent/i)).toBeTruthy();
      expect(rightsEvaluate.getByText(/UNCERTAIN_COMMERCIAL_PERMISSION stays blocking/i)).toBeTruthy();
      const retentionDelete = within(screen.getByRole('region', { name: /unforced retention delete/i }));
      expect(retentionDelete.getByText(/keeps mayDelete false/i)).toBeTruthy();
      expect(retentionDelete.getByText(/client authorisedPolicy true is not sent/i)).toBeTruthy();
      expect(retentionDelete.getByText(/empty kind stays unauthorised/i)).toBeTruthy();
      expect(api.fetchMatchWorkspace).toHaveBeenCalledTimes(1);
    });

    it('posts decode challengers, GPU default flag, and identity cluster without leftover pyav default, enabled true, or selectedSemantic', async () => {
      stubPitchCanvas();
      vi.mocked(api.fetchMatches).mockResolvedValue([readyMatch('match-a', 'Match A')]);
      vi.mocked(api.fetchMatchWorkspace).mockResolvedValue(loadedWorkspace('match-a', 'Match A'));
      const fetchMock = vi.fn((input: RequestInfo, init?: RequestInit) => {
        ...
        if (url.endsWith('/api/decode/challengers') && init?.method === 'POST') {
          return Promise.resolve({
            ok: true,
            json: async () => ({
              pyav: { name: 'pyav', default: false, enabled: false, role: 'challenger' },
        ...
        if (url.endsWith('/api/flags/gpu_default/enabled') && init?.method === 'POST') {
          return Promise.resolve({
            ok: true,
            json: async () => ({ name: 'gpu_default', enabled: false }),
          } as Response);
        }
        if (url.endsWith('/api/identity/clusters/0') && init?.method === 'POST') {
          return Promise.resolve({
            ok: true,
            json: async () => ({
              clusterId: 0,
      ...
      fireEvent.click(screen.getByRole('button', { name: /request GPU default flag/i }));
      await waitFor(() => {
        expect(fetchMock.mock.calls.some(([url, init]) => (
          String(url).endsWith('/api/flags/gpu_default/enabled')
          && init?.method === 'POST'
      ...
      fireEvent.click(screen.getByRole('button', { name: /request identity cluster/i }));
      await waitFor(() => {
        expect(fetchMock.mock.calls.some(([url, init]) => (
      ...
      expect(fetchMock.mock.calls.some(([url, init]) => (
        String(url).includes('/api/decode/challengers')
        && Boolean(init?.body && (String(init.body).includes('pyav') || String(init.body).includes('default')))
      ))).toBe(false);
      expect(fetchMock.mock.calls.some(([url, init]) => (
        String(url).includes('/api/flags/gpu_default/enabled')
        && Boolean(init?.body && String(init.body).includes('enabled'))
      ))).toBe(false);
      expect(fetchMock.mock.calls.some(([url, init]) => (
      ...
      expect(screen.queryByText(/empty paths stay non-product/i)).toBeNull();
      expect(screen.queryByText(/UNCERTAIN_COMMERCIAL_PERMISSION stays blocking/i)).toBeNull();
      expect(screen.queryByText(/empty kind stays unauthorised/i)).toBeNull();
      const challengers = within(screen.getByRole('region', { name: /unforced decode challengers/i }));
      expect(challengers.getByText(/keep pyav enabled false/i)).toBeTruthy();
      expect(challengers.getByText(/client pyav default true is not sent/i)).toBeTruthy();
      expect(challengers.getByText(/selected decode stays opencv/i)).toBeTruthy();
      const gpuFlag = within(screen.getByRole('region', { name: /unforced GPU default flag/i }));
      expect(gpuFlag.getByText(/Posted GPU default flag keeps enabled false/i)).toBeTruthy();
      expect(gpuFlag.getByText(/client POST enabled true is not sent/i)).toBeTruthy();
      expect(gpuFlag.getByText(/Posted flag stays off GPU promotion/i)).toBeTruthy();
      const cluster = within(screen.getByRole('region', { name: /unforced identity cluster/i }));
    ...
    it('posts split scores, decode frames, and providers without leftover detectorScore, cuda, or enabled cloud', async () => {
      stubPitchCanvas();
      vi.mocked(api.fetchMatches).mockResolvedValue([readyMatch('match-a', 'Match A')]);
      vi.mocked(api.fetchMatchWorkspace).mockResolvedValue(loadedWorkspace('match-a', 'Match A'));
      const fetchMock = vi.fn((input: RequestInfo, init?: RequestInit) => {
        ...
        if (url.endsWith('/api/quantities/scores') && init?.method === 'POST') {
          return Promise.resolve({
            ok: true,
            json: async () => ({
              detectorScore: null,
        ...
        if (url.endsWith('/api/decode/frames') && init?.method === 'POST') {
          return Promise.resolve({
            ok: true,
            json: async () => ({
              backend: 'fixture',
      ...
      render(<App />);
      await screen.findByText('Match A', { selector: 'header span' });
      await waitFor(() => expect(screen.getByRole('button', { name: /request split scores/i })).toBeTruthy());
      expect(fetchMock.mock.calls.some(([url, init]) => String(url).endsWith('/api/quantities/scores') && init?.method === 'POST')).toBe(false);
      expect(fetchMock.mock.calls.some(([url, init]) => String(url).endsWith('/api/decode/frames') && init?.method === 'POST')).toBe(false);
      expect(fetchMock.mock.calls.some(([url, init]) => String(url).endsWith('/api/providers') && init?.method === 'POST')).toBe(false);
      fireEvent.click(screen.getByRole('button', { name: /request split scores/i }));
      await waitFor(() => {
        expect(fetchMock.mock.calls.some(([url, init]) => (
      ...
      fireEvent.click(screen.getByRole('button', { name: /request decode frames/i }));
      await waitFor(() => {
        expect(fetchMock.mock.calls.some(([url, init]) => (
          String(url).endsWith('/api/decode/frames')
          && init?.method === 'POST'
      ...
      fireEvent.click(screen.getByRole('button', { name: /request provider roster/i }));
      await waitFor(() => {
        expect(fetchMock.mock.calls.some(([url, init]) => (
      ...
      expect(fetchMock.mock.calls.some(([url, init]) => (
        String(url).includes('/api/quantities/scores')
        && Boolean(init?.body && (String(init.body).includes('detectorScore') || String(init.body).includes('0.81')))
      ))).toBe(false);
      expect(fetchMock.mock.calls.some(([url, init]) => (
        String(url).includes('/api/decode/frames')
        && Boolean(init?.body && (String(init.body).includes('cuda') || String(init.body).includes('pyav')))
      ))).toBe(false);
      expect(fetchMock.mock.calls.some(([url, init]) => (
        String(url).includes('/api/providers')
        && Boolean(init?.body && (String(init.body).includes('enabled') || String(init.body).includes('cloud')))
      ))).toBe(false);
      expect(fetchMock.mock.calls.some(([url]) => String(url).includes('/api/quota'))).toBe(false);
      expect(fetchMock.mock.calls.some(([url]) => String(url).includes('/api/training/admit'))).toBe(false);
      expect(fetchMock.mock.calls.some(([url]) => String(url).includes('/api/receipts/promotion'))).toBe(false);
      ...
      expect(fetchMock.mock.calls.some(([url]) => String(url).includes('/api/geometry/landmarks'))).toBe(false);
      expect(fetchMock.mock.calls.some(([url]) => String(url).includes('/api/geometry/preview'))).toBe(false);
      expect(screen.queryByText(/a detector score is not a calibrated probability/i)).toBeNull();
      expect(screen.queryByText(/production decode stays on the fixture FrameSource/i)).toBeNull();
      expect(screen.queryByText(/language providers stay disabled by default/i)).toBeNull();
      expect(screen.queryByText(/Posted leftover geometry keeps evaluation.accepted false/i)).toBeNull();
      const scores = within(screen.getByRole('region', { name: /unforced split scores/i }));
      expect(scores.getByText(/Posted split scores keep detectorScore null/i)).toBeTruthy();
      expect(scores.getByText(/client POST detectorScore 0.81 is not sent/i)).toBeTruthy();
      expect(scores.getByText(/Posted calibratedProbability stays null/i)).toBeTruthy();
      const decodeFrames = within(screen.getByRole('region', { name: /unforced decode frames/i }));
      expect(decodeFrames.getByText(/Posted decode frames keep backend fixture/i)).toBeTruthy();
      expect(decodeFrames.getByText(/client POST pyav cuda frames are not sent/i)).toBeTruthy();
      expect(decodeFrames.getByText(/Posted gpuPromoted stays false/i)).toBeTruthy();
      const providers = within(screen.getByRole('region', { name: /unforced provider roster/i }));
      expect(providers.getByText(/Posted provider roster keeps the default disabled/i)).toBeTruthy();
      expect(providers.getByText(/client POST enabled cloud is not sent/i)).toBeTruthy();
      expect(providers.getByText(/Posted local and cloud routes stay disabled/i)).toBeTruthy();
      expect(api.fetchMatchWorkspace).toHaveBeenCalledTimes(1);
    });

    it('posts dependencies, ground contact, and dossier release without leftover fashionableOnly true, airborne true, or nativeCode approved', async () => {
      stubPitchCanvas();
      vi.mocked(api.fetchMatches).mockResolvedValue([readyMatch('match-a', 'Match A')]);
      vi.mocked(api.fetchMatchWorkspace).mockResolvedValue(loadedWorkspace('match-a', 'Match A'));
      const fetchMock = vi.fn((input: RequestInfo, init?: RequestInit) => {
        ...
        if (url.endsWith('/api/dependencies') && init?.method === 'POST') {
          return Promise.resolve({
            ok: true,
            json: async () => ({
        ...
        if (url.endsWith('/api/geometry/contact') && init?.method === 'POST') {
          return Promise.resolve({
            ok: true,
            json: async () => ({
              imageX: 5.0,
        ...
        if (url.endsWith('/api/dossier/release') && init?.method === 'POST') {
          return Promise.resolve({
            ok: true,
            json: async () => ({
      ...
      render(<App />);
      await screen.findByText('Match A', { selector: 'header span' });
      await waitFor(() => expect(screen.getByRole('button', { name: /request dependency register/i })).toBeTruthy());
      expect(fetchMock.mock.calls.some(([url, init]) => String(url).endsWith('/api/dependencies') && init?.method === 'POST')).toBe(false);
      expect(fetchMock.mock.calls.some(([url, init]) => String(url).endsWith('/api/geometry/contact') && init?.method === 'POST')).toBe(false);
      expect(fetchMock.mock.calls.some(([url, init]) => String(url).endsWith('/api/dossier/release') && init?.method === 'POST')).toBe(false);
      fireEvent.click(screen.getByRole('button', { name: /request dependency register/i }));
      await waitFor(() => {
        expect(fetchMock.mock.calls.some(([url, init]) => (
      ...
      fireEvent.click(screen.getByRole('button', { name: /request ground contact/i }));
      await waitFor(() => {
        expect(fetchMock.mock.calls.some(([url, init]) => (
          String(url).endsWith('/api/geometry/contact')
          && init?.method === 'POST'
          && init.body === '{}'
        ))).toBe(true);
      });
      fireEvent.click(screen.getByRole('button', { name: /request release dossier/i }));
      await waitFor(() => {
        expect(fetchMock.mock.calls.some(([url, init]) => (
      ...
      expect(fetchMock.mock.calls.some(([url, init]) => (
        String(url).includes('/api/dependencies')
        && Boolean(init?.body && String(init.body).includes('fashionableOnly'))
      ))).toBe(false);
      expect(fetchMock.mock.calls.some(([url, init]) => (
        String(url).includes('/api/geometry/contact')
        && Boolean(init?.body && (String(init.body).includes('airborne') || String(init.body).includes('bbox')))
      ))).toBe(false);
      expect(fetchMock.mock.calls.some(([url, init]) => (
        String(url).includes('/api/dossier/release')
        && Boolean(init?.body && (String(init.body).includes('nativeCode') || String(init.body).includes('approved')))
      ))).toBe(false);
      expect(fetchMock.mock.calls.some(([url]) => String(url).includes('/api/quota'))).toBe(false);
      expect(fetchMock.mock.calls.some(([url]) => String(url).includes('/api/training/admit'))).toBe(false);
      expect(fetchMock.mock.calls.some(([url]) => String(url).includes('/api/receipts/promotion'))).toBe(false);
      ...
      expect(screen.queryByText(/does not treat box centre as the foot/i)).toBeNull();
      expect(screen.queryByText(/client fashionableOnly true is not sent/i)).toBeNull();
      expect(screen.queryByText(/Posted provider roster keeps the default disabled/i)).toBeNull();
      const dependencies = within(screen.getByRole('region', { name: /unforced dependency register/i }));
      expect(dependencies.getByText(/Posted dependency register keeps fashionableOnly false/i)).toBeTruthy();
      expect(dependencies.getByText(/client POST fashionableOnly true is not sent/i)).toBeTruthy();
      expect(dependencies.getByText(/Posted OpenCV rollbackPath stays fixture_frame_source/i)).toBeTruthy();
      const contact = within(screen.getByRole('region', { name: /unforced ground contact/i }));
      expect(contact.getByText(/Posted ground contact keeps boxCentreIsFoot false/i)).toBeTruthy();
      expect(contact.getByText(/client POST airborne true is not sent/i)).toBeTruthy();
      expect(contact.getByText(/Posted contact stays a foot estimate, not box centre/i)).toBeTruthy();
      const dossier = within(screen.getByRole('region', { name: /unforced release dossier/i }));
      expect(dossier.getByText(/Posted release dossier keeps nativeCode gated_inert/i)).toBeTruthy();
      expect(dossier.getByText(/client POST nativeCode approved is not sent/i)).toBeTruthy();
      expect(dossier.getByText(/Posted deploymentBoundary stays loopback/i)).toBeTruthy();
      expect(api.fetchMatchWorkspace).toHaveBeenCalledTimes(1);
    });
    ...
    it('posts leftover heatmap, leftover search, and leftover signed access without leftover identityContinuous true, matchId-less events, or leftover token', async () => {
      ...
      expect(fetchMock.mock.calls.some(([url]) => String(url).includes('/api/cache/tenancy'))).toBe(false);
      expect(fetchMock.mock.calls.some(([url, init]) => String(url).includes('/api/geometry/preview') && init?.method === 'POST')).toBe(false);
    ...
    it('loads stored match setup without certifying automation', async () => {
      ...
      const fetchMock = vi.fn((input: RequestInfo, init?: RequestInit) => {
        ...
        if (url.includes('/api/matches/match-a/setup/preview') && (!init?.method || init.method === 'GET')) {
          return Promise.resolve({
            ok: true,
            json: async () => ({
              residualP95M: null,
